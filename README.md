...
| Mediator Live Data | It is a subclass of `MutableLiveData`.<br /> Here we can combite the mutable live data from two different or multiple different data sources.<br /> Example of the scenario include when we are displaying a list of users from server and from the local database. <br /> But the data displayed in the presentation layer is a single list if users | [View](app/src/main/java/com/demo/code/liveData/mutableLiveData/fragments/MediatorLiveDataFragment.kt) , [ViewModel](app/src/main/java/com/demo/code/liveData/mutableLiveData/vm/MediatorLiveDataViewModel.kt) |
